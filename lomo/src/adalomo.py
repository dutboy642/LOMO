...
from utils import DynamicLossScaler
...
class AdaLomo(Optimizer):
    ...
    def __init__(
        ...
        self.zero3_enabled = False
        if hasattr(model, 'parameters'):
            p0 = list(self.model.parameters())[0]
            if hasattr(p0, 'ds_tensor'):  # zero3 is enabled
                self.zero3_enabled = True

        # check if fp16 is enabled
        if hasattr(p0, 'dtype') and p0.dtype == torch.float16:
            self.loss_scaler = DynamicLossScaler(
                init_scale=2 ** 16,
            ) 
            # Warning: LOMO requires grad norm for stable fp16 training usually, 
            # but we allow skipping it for DeepSpeed compatibility.
        else:
